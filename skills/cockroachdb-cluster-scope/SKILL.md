...
description: "Applies whenever a request names a specific CockroachDB cluster, a cockroachdb-cloud tool call would pass cluster_id, or results seem to come from a different cluster than requested. Rule: if the power's mcp.json sets an mcp-cluster-id header, the connection is locked to that cluster. Never edit mcp.json (or any MCP config) to change or remove the header, and never offer to. Never retry with another cluster_id or look for another route. Tell the user which cluster the connection is scoped to, stop, and explain how they can change it themselves."
...
- A tool returns an error like `cluster_id is set in your MCP config; omit the cluster_id argument`.
...
1. **Don't present the pinned cluster's data as the requested cluster's.** If a call returned data from the pinned cluster, say so.
...
- On a scoped connection, leave out the `cluster_id` argument.
