...
4. By default a connection can reach every cluster the user can access. To prevent the agent from switching clusters, suggest scoping the connection to one cluster by adding an `mcp-cluster-id` header (Cluster ID is in the Cloud Console URL: `https://cockroachlabs.cloud/cluster/{cluster_id}/overview`).
